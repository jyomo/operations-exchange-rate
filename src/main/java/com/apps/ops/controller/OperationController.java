...
public class OperationController {
	...
	public ResponseJson transfer(@RequestBody OperationRequest operationRequest) {
		...
		if (sourceAccount.isPresent() && targetAccount.isPresent() ) {
			...
			logger.info(String.format("cuentaOrigen [%s]", sourceAccount.get().toString()));
			logger.info(String.format("cuentaDestino [%s]", targetAccount.get().toString()));
			
			MovementChange movementChange= new MovementChange(operationRequest.getAmountMovement(), operationRequest.getCurrencyMovement());
			logger.info(String.format("movementChange [%s]", movementChange.toString()));
