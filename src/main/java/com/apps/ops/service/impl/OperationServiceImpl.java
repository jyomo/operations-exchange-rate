		...
		if (sourceAccountOptional.isPresent() && targetAccountOptional.isPresent()) {
			...
			if (sourceAccount.getCurrency().equals(targetAccount.getCurrency())) {
				if (sourceAccount.getCurrency().equals(movementChange.getCurrency())) {
					...
					logger.info(
							"CASO 1. MonedaOrigen [{}] , MonedaDestino [{}] y  monedaTransferencia [{}] son iguales",
					...
					logger.info(
							"CASO 2. MonedaOrigen [{}] , MonedaDestino [{}] iguales pero monedaTransferencia [{}] diferente",
							sourceAccount.getCurrency(), targetAccount.getCurrency(), movementChange.getCurrency());
				...
				if (coreExchange == null) {
					return new ResponseJson(Constants.HTTP_BAD_REQUEST_STATUS,
							Constants.VALIDACION_OPERACION_NO_PERMITIDA);
				}
